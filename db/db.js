...
const connection = mysql.createConnection({
    host: 'localhost',
    user: 'root',
    password: 'TXTUSERÑ57', //En mi caso me conecto a mysql-server con estos datos
    port: 3306,
    // database: 'movies_db'
});
...
connection.connect((err) => {
    ...
    // Creamos la consulta
    const sqlCreatedb = 'CREATE DATABASE IF NOT EXISTS podcasts_db';
    ...
    connection.query(sqlCreatedb, (err, results) => {
        ...
        connection.changeUser({ database: 'podcasts_db' }, (err) => {            
            //error
            if (err) {
                console.error('Error al cambiar a la base de datos podcasts_db:', err);
                return;
            }
